var minTimeToVisitAllPoints = function(points) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        if (points[i][0] === points[i+1][0]) {
            time += Math.abs(points[i+1][1] - points[i][1]);
        } else if (points[i][1] === points[i+1][1]) {
            time += Math.abs(points[i+1][0] - points[i][0]);
        } else {
            const diff = Math.abs(points[i+1][0] - points[i][0]);
           
            if (points[i+1][0] > points[i][0]) {
                 time += diff + Math.abs(points[i+1][1] - (diff + points[i][1]));
            } else {
                time += diff + Math.abs(points[i][1] - (diff + points[i+1][1]));
            }
        }
    }
